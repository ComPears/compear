...
const siteUrl = (process.env.VITE_SITE_URL || 'https://compears.shop').replace(/\/$/, '');
...
for (const country of ['uk', 'nl']) {
  ...
  const homeAlternates = [{ lang: 'en-GB', href: `${siteUrl}/uk` }, { lang: 'nl-NL', href: `${siteUrl}/nl` }, { lang: 'x-default', href: `${siteUrl}/uk` }];
  writePage(homeRoute, { title: homeTitle, description: homeDescription, url: `${siteUrl}${homeRoute}`, locale, alternates: homeAlternates, schema: { '@context': 'https://schema.org', '@type': 'WebSite', name: 'ComPear', url: `${siteUrl}/${country}`, potentialAction: { '@type': 'SearchAction', target: `${siteUrl}/${country}/search?q={search_term_string}`, 'query-input': 'required name=search_term_string' } } }, `<h1>${homeTitle.replace(' | ComPear', '')}</h1><p>${homeDescription}</p><nav><ul>${homeLinks}</ul></nav>`);
  sitemap.push(`${siteUrl}${homeRoute}`, `${siteUrl}/${country}/how-it-works`);

  writePage(`/${country}/how-it-works`, { title: `How ComPear compares prices | ComPear`, description: 'Learn how ComPear matches products, checks price freshness and presents independent supermarket comparisons.', url: `${siteUrl}/${country}/how-it-works`, locale, alternates: [{ lang: 'en-GB', href: `${siteUrl}/uk/how-it-works` }, { lang: 'nl-NL', href: `${siteUrl}/nl/how-it-works` }] }, '<h1>How ComPear compares prices</h1><p>We compare product identity, variant and package size, show when prices were checked, and link to the retailer source.</p>');
  ...
  for (const category of categories.filter((value) => value !== 'Other')) {
    ...
    writePage(route, { title: `${category} price comparison | ComPear`, description, url: `${siteUrl}${route}`, locale, alternates: [{ lang: 'en-GB', href: `${siteUrl}/uk/categories/${categorySlug(category)}` }, { lang: 'nl-NL', href: `${siteUrl}/nl/categories/${categorySlug(category)}` }], schema: { '@context': 'https://schema.org', '@type': 'ItemList', name: `${category} price comparison`, numberOfItems: groups.length, itemListElement: groups.map(([slug, offers], index) => ({ '@type': 'ListItem', position: index + 1, name: offers[0].productName, url: `${siteUrl}/${country}/products/${slug}` })) } }, `<h1>${escapeHtml(category)} price comparison</h1><p>${description}</p><ul>${links}</ul>`);
    sitemap.push(`${siteUrl}${route}`);
  }
