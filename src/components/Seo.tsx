...
const SITE_URL = (import.meta.env.VITE_SITE_URL || 'https://compears.shop').replace(/\/$/, '');
...
export function Seo({
  ...
  useEffect(() => {
    ...
    if (hasEquivalentCountryPage) {
      setLink('alternate', `${SITE_URL}/uk${alternatePath}`, 'en-GB');
      setLink('alternate', `${SITE_URL}/nl${alternatePath}`, 'nl-NL');
      setLink('alternate', `${SITE_URL}/uk${alternatePath}`, 'x-default');
    } else {
